...
Vite apps must deploy the **build output** (`dist/`), not the repo root. If you see requests for `/src/main.tsx` or a URL containing `%BASE_URL%`, the wrong files are being published — `index.html` at the project root is only for `npm run dev`; production HTML and bundles are generated by `npm run build`.

- **Recommended:** enable **Pages → Build and deployment → Source: GitHub Actions**, then push to `main`. The workflow in `.github/workflows/deploy-github-pages.yml` runs `npm ci` + `npm run build` and uploads `dist/`.
- **Manual:** run `npm run build` locally and upload **everything inside `dist/`** to your Pages branch or hosting root (not `index.html` + `src/` from the repository).

`vite.config.ts` sets `base: './'` so asset URLs work under `https://<user>.github.io/<repo>/`.
